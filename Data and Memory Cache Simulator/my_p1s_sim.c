...
{
    int blockSizeInWords, numberOfSets, blocksPerSet;
    blockSizeInWords = numberOfSets = blocksPerSet = 0;
    ...
    while (state.pc < state.numMemory)
    {
        ...
        else if (opcode == 2)   //LW
        {
            ...
            state.reg[get_regB(&state, state.pc)] = cache_access(num, 0, 0); //state.mem[num];
            state.pc++;
        }
        else if (opcode == 3)       //SW
        {
            ...
            cache_access(num, 1, state.reg[get_regB(&state, state.pc)]);
            state.pc++;
        }
        else if (opcode == 4)
        {
